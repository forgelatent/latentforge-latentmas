...
- kalshi-pull `[LOADED: yes | VALID: yes]`
  ...
  - **September 19, 2026 (fourth session): empty-answer ticket closed, commit `d7c8d09`.** File hash of record `2549a452...` (69 lines; the first hash recorded for this script in this manifest; supersedes `8fc24ac8`). **Failure contract:** exit 1 = could not reach Kalshi (exception); exit 2 = Kalshi answered but the answer was unusable (unrecognised shape, or markets empty or not a list). On either, nothing is written to the output path; both checks precede the write. The wrapper retries on any non-zero exit. Verified on host by offline harness (3 of 9 before, 9 of 9 after) and one live run. First run through the launchd wrapper September 20, 2026: SUCCESS on attempt 1, 1,000 markets saved, first-page-only NOTE logged (success path only; how the wrapper treats exit 2 is not yet verified). Fourth same-session fix; cold re-read owed.
