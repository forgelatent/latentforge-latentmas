...
Last meaningful update: September 21, 2026
...
- kalshi-pull `[LOADED: yes | VALID: yes]`
  ...
  - **September 19, 2026 (fourth session): empty-answer ticket closed, commit `d7c8d09`.** File hash of record `2549a452...` (69 lines; the first hash recorded for this script in this manifest; supersedes `8fc24ac8`). **Failure contract:** exit 1 = could not reach Kalshi (exception); exit 2 = Kalshi answered but the answer was unusable (unrecognised shape, or markets empty or not a list). On either, nothing is written to the output path; both checks precede the write. The wrapper retries on any non-zero exit. Verified on host by offline harness (3 of 9 before, 9 of 9 after) and one live run. First run through the launchd wrapper September 20, 2026: SUCCESS on attempt 1, 1,000 markets saved, first-page-only NOTE logged (success path only; by reading the wrapper on September 21, any non-zero exit is logged as FAILED with its code and retried, so exit 2 is treated like exit 1; not yet observed in a real run). Fourth same-session fix; cold re-read completed September 21, 2026: 3 of 3 PASS, no defects, five observations (see `incident_ledger.md` September 21 second entry).
...
- compression-researcher `[LOADED: yes | VALID: limited | scope: research-only, no downstream impact]`
  ...
  - **First scheduled night under `2832d02`, September 20, 2026 (facts only; the label and the BLOCKING tag above are unchanged; one night):** attempt 1 started 02:02:59 PDT, was suspended under a closed lid, and was stamped FAILED (exit 1) at 10:25:23; before the fix this night would have been stamped SUCCESS with error text on the output path. The wrapper's 300-second wait lasted 3h12m; attempt 2 succeeded at 13:38:15 in 35 seconds. Output `2026-09-20.md` is real (6,349 bytes, three `SUGGESTION`s): 30 real files and 104 junk files on disk as of September 20. The `caffeinate -i` assertion did not hold with the lid closed; whether it holds with the lid open is still unproven. The output exists only because the lid was opened that day. Detail in `incident_ledger.md` September 20 entry. *Reproducers:* `grep -E "Sep 20 .*(Attempt [0-9] of 3|SUCCESS on|FAILED on)" research/suggestions/cron.log` returns four lines; `grep -l "SUGGESTION" research/suggestions/*.md | wc -l` returned 30 and `grep -L "SUGGESTION" research/suggestions/*.md | wc -l` returned 104 on September 20.
  - **Second scheduled night under `2832d02`, September 21, 2026 (facts only; the label and the BLOCKING tag above are unchanged; two nights):** run as a Founder test with the lid open and the charger in (Founder statement), no settings changed. Attempt 1 started 02:00:53 PDT and was stamped SUCCESS at 02:01:32 (39 seconds): the first scheduled first-attempt success under the fix. Output `2026-09-21.md` is real (6,034 bytes, three `SUGGESTION`s): 31 real files and 104 junk files on disk as of September 21. **This night does not show that the `caffeinate -i` assertion works:** research-sweep, which has no assertion, also ran without suspension the same night (12 seconds), so what differed from September 20 is the lid. Whether the assertion holds when the host attempts sleep mid-call is still unproven. Detail in `incident_ledger.md` September 21 entry. *Reproducers:* `stat -f "%z" research/suggestions/2026-09-21.md` returns 6034; `grep -l "SUGGESTION" research/suggestions/*.md | wc -l` returned 31 and `grep -L "SUGGESTION" research/suggestions/*.md | wc -l` returned 104 on September 21.
...
- mode1-loader `[LOADED: yes | VALID: yes]`
  ...
  - Current data state: **14 LIVE, 1 RETIRED** (scheduled run of September 21, 2026: `RETIRED_PRESENT`, exit 1, a success tier; previously 15 LIVE, 0 RETIRED, verified September 14, 2026 — first exit 0 since the June 30 v1 retirements). The retired market is `will-bitcoin-reach-82pt5k-in-september-2026`, `cause_of_death: closed`, about nine days before the month-end settlement expected on September 14; how it resolved was not read. **Downstream consumers must still handle a shrinking live count:** Bolsonaro was 19 days from resolving as of September 14, and rule 3d triggers next-version selection below 10 live, urgent below 6. *Reproducer:* `grep "Sep 21" experiments/benchmark/mode1/cron.log` shows the `live=14 retired=1` line.
