...
DATE_WINDOWS = [
    ...
    ("2026-09-15T00:00:00Z", "2026-09-15T06:00:00Z"),
    ("2026-09-15T06:00:00Z", "2026-09-15T12:00:00Z"),
    ("2026-09-15T12:00:00Z", "2026-09-15T18:00:00Z"),
    ("2026-09-15T18:00:00Z", "2026-09-16"),
...
def walk_window(start, end, all_events):
    """Probe a window; pull it if it fits, else bisect at the time midpoint
    and recurse. Abort loudly if a window at the 1-hour floor still
    overflows - that would mean something new is going on."""
    n = probe_window(start, end)
    if n >= PAGE_SIZE:
        s_dt, e_dt = _parse_iso(start), _parse_iso(end)
        if (e_dt - s_dt).total_seconds() <= MIN_WINDOW_SECONDS:
            raise RuntimeError(
                f"Window [{start}, {end}) overflows the offset ceiling at the "
                f"one-hour floor - 2000+ events ending in a single hour. "
                f"Investigate before pulling. No output written.")
        mid = _fmt_iso(s_dt + (e_dt - s_dt) / 2)
        print(f"  window [{start}, {end}) overflows -> splitting at {mid}")
        walk_window(start, mid, all_events)
        walk_window(mid, end, all_events)
        return
...
def fetch_events_windowed():
    """Pull every live event via half-open date windows, auto-splitting
    any window that overflows the offset ceiling."""
    all_events = {}
    for start, end in DATE_WINDOWS:
        walk_window(start, end, all_events)
    return all_events
